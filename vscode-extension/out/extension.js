...
const photonDriftCLI_1 = require("./photonDriftCLI");
const adrTreeProvider_1 = require("./adrTreeProvider");
const driftStatusBar_1 = require("./driftStatusBar");
...
function activate(context) {
    ...
    cli = new photonDriftCLI_1.PhotonDriftCLI();
    // Initialize tree provider for ADR files
    adrTreeProvider = new adrTreeProvider_1.ADRTreeProvider(context);
    vscode.window.registerTreeDataProvider('photondriftAdrTree', adrTreeProvider);
    // Initialize status bar
    statusBar = new driftStatusBar_1.DriftStatusBar();
    context.subscriptions.push(statusBar);
    // Initialize diagnostic provider for inline warnings
    diagnosticProvider = new diagnosticProvider_1.ADRDiagnosticProvider();
    context.subscriptions.push(diagnosticProvider);
    // Register completion provider for ADR templates
    const completionProvider = new completionProvider_1.ADRCompletionProvider();
    context.subscriptions.push(vscode.languages.registerCompletionItemProvider({ language: 'markdown' }, completionProvider, '#' // Trigger on # for ADR headers
    ));
    // Register commands
...
function registerCommands(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('photondrift.init', async () => {
        try {
    ...
    // Create new ADR
    context.subscriptions.push(vscode.commands.registerCommand('photondrift.createAdr', async (uri) => {
        try {
    ...
    // Run drift detection
    context.subscriptions.push(vscode.commands.registerCommand('photondrift.runDriftDetection', async () => {
        try {
            const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
            if (!workspaceFolder) {
                vscode.window.showErrorMessage('No workspace folder open');
            ...
            if (results.length > 0) {
                const action = await vscode.window.showInformationMessage(`Found ${results.length} drift item(s)`, 'View Results', 'Generate Report');
    ...
    // Generate ADR index
    context.subscriptions.push(vscode.commands.registerCommand('photondrift.generateIndex', async () => {
        try {
            const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
            if (!workspaceFolder) {
                vscode.window.showErrorMessage('No workspace folder open');
    ...
    // Show ADR inventory
    context.subscriptions.push(vscode.commands.registerCommand('photondrift.showInventory', async () => {
        try {
            const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
            if (!workspaceFolder) {
                vscode.window.showErrorMessage('No workspace folder open');
    ...
    context.subscriptions.push(vscode.commands.registerCommand('photondrift.proposeAdr', async () => {
        try {
    ...
    // Open settings
    context.subscriptions.push(vscode.commands.registerCommand('photondrift.openSettings', () => {
        vscode.commands.executeCommand('workbench.action.openSettings', 'photondrift');
    }));
}
function setupFileWatching(context) {
    // Watch for ADR file changes
...
async function showDriftResults(results) {
...
async function generateDriftReport(results) {
    try {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            return;
...
async function showInventoryWebview(inventory) {
    const panel = vscode.window.createWebviewPanel('photondriftInventory', 'ADR Inventory', vscode.ViewColumn.Two, {
        enableScripts: true,
        retainContextWhenHidden: true
    });
    panel.webview.html = generateInventoryHTML(inventory);
}
async function showProposalWebview(proposal) {
    const panel = vscode.window.createWebviewPanel('photondriftProposal', 'ADR Proposal', vscode.ViewColumn.Two, {
        enableScripts: true,
        retainContextWhenHidden: true
    });
    panel.webview.html = generateProposalHTML(proposal);
}
function formatDriftResults(results) {
