...
use crate::plugins::{
    Plugin, PluginMetadata, PluginCapability, PluginContext, PluginResponse,
    PluginValidator, SecurityPolicy, SandboxManager, PLUGIN_API_VERSION,
    MAX_PLUGIN_EXECUTION_TIME_MS, MAX_PLUGIN_MEMORY_BYTES
};
...
impl PluginManager {
    ...
    pub fn load_plugin<P: AsRef<Path>>(&mut self, plugin_path: P) -> Result<String, PluginLoadError> {
    ...
    pub fn execute_plugin_command(
        ...
        let result = plugin.execute(command, params)
            .map_err(|e| PluginLoadError::ConfigurationError(e.to_string()))?;
        
        let execution_time = start_time.elapsed();
        if execution_time > max_duration {
            warn!("Plugin {} execution time exceeded limit: {}ms", plugin_id, execution_time.as_millis());
            return Err(PluginLoadError::ExecutionTimeout(execution_time.as_millis() as u64));
        }
        
        debug!("Plugin command executed in {}ms", execution_time.as_millis());
        Ok(result)
    }
    ...
    pub fn auto_load_plugins(&mut self) -> Result<Vec<String>, PluginLoadError> {
        ...
        for path in plugin_paths {
            match self.should_auto_load(&path) {
                ...
                Err(e) => {
                    warn!("Error checking auto-load for plugin {}: {}", path.display(), e);
                }
            }
        }
    ...
    pub fn update_plugin_config(&mut self, plugin_id: &str, config: PluginConfig) -> Result<(), PluginLoadError> {
    ...
    fn load_plugin_binary(&self, _plugin_path: &Path, metadata: PluginMetadata) -> Result<Box<dyn Plugin>, PluginLoadError> {
        // This is a simplified implementation
    ...
    fn get_plugin_config(&self, plugin_id: &str) -> PluginConfig {
        self.plugin_configs.get(plugin_id).cloned().unwrap_or_else(|| {
            PluginConfig {
                enabled: true,
                auto_load: false,
    ...
    fn is_plugin_file(&self, path: &Path) -> bool {
        if let Some(extension) = path.extension() {
            matches!(extension.to_str(), Some("dll") | Some("so") | Some("dylib") | Some("wasm"))
        } else {
            false
        }
    }
...
impl Plugin for ExamplePlugin {
    ...
    fn capabilities(&self) -> Vec<PluginCapability> {
        vec![PluginCapability::DriftAnalysis, PluginCapability::TemplateProvider]
    }
