...
impl IndexCommand {
    pub fn execute(&self, config: &Config) -> Result<()> {
        ...
        if let Some(ref status_filter) = self.status_filter {
            ...
            log::info!("Filtered to {} ADRs with status '{}'", adr_entries.len(), status_filter);
        }
        ...
        // Write index file
        std::fs::write(&output_path, index_content)
    ...
    fn parse_adr_entry(&self, file_path: &std::path::Path, adr_dir: &PathBuf) -> Result<AdrIndexEntry> {
        let adr_doc = AdrParser::parse_file(file_path)?;
        
        // Calculate relative path from ADR directory
        let relative_path = file_path.strip_prefix(adr_dir)
    ...
    fn sort_entries(&self, entries: &mut Vec<AdrIndexEntry>) {
        match self.sort.as_str() {
            ...
            "status" => {
                entries.sort_by(|a, b| {
                    let status_order = self.get_status_order(&a.status).cmp(&self.get_status_order(&b.status));
    ...
    fn generate_default_index(&self, entries: &[AdrIndexEntry], adr_dir: &PathBuf) -> Result<String> {
        ...
        content.push_str(&format!("This index contains {} Architecture Decision Records.\n\n", entries.len()));
    ...
    fn generate_custom_index(&self, entries: &[AdrIndexEntry], template_path: &PathBuf, adr_dir: &PathBuf) -> Result<String> {
        ...
        content = content.replace("{{TIMESTAMP}}", &chrono::Utc::now().format("%Y-%m-%d %H:%M:%S UTC").to_string());
    ...
    fn get_status_counts(&self, entries: &[AdrIndexEntry]) -> Vec<(String, usize)> {
        ...
        sorted_counts.sort_by(|a, b| {
            self.get_status_order(&a.0).cmp(&self.get_status_order(&b.0))
        });
    ...
    fn get_status_badge(&self, status: &str) -> String {
        match status.to_lowercase().as_str() {
            "accepted" => "![Accepted](https://img.shields.io/badge/Status-Accepted-green)".to_string(),
            "proposed" => "![Proposed](https://img.shields.io/badge/Status-Proposed-yellow)".to_string(),
            "rejected" => "![Rejected](https://img.shields.io/badge/Status-Rejected-red)".to_string(),
            "deprecated" => "![Deprecated](https://img.shields.io/badge/Status-Deprecated-orange)".to_string(),
            "superseded" => "![Superseded](https://img.shields.io/badge/Status-Superseded-lightgrey)".to_string(),
            _ => format!("![{}](https://img.shields.io/badge/Status-{}-blue)", status, status),
        }
    }
}
...
mod tests {
    ...
    fn test_index_with_adr_files() {
        ...
        create_test_adr(&adr_dir, "0001-use-react.md", r#"---
...
"#);
        ...
        create_test_adr(&adr_dir, "0002-use-postgres.md", r#"---
...
"#);
    ...
    fn test_index_with_badges() {
        ...
        create_test_adr(&adr_dir, "test.md", r#"---
title: "Test ADR"
status: accepted
---
...
"#);
    ...
    fn test_extract_number() {
        ...
        assert_eq!(cmd.extract_number(&Some("001".to_string()), &PathBuf::from("test.md")), Some(1));
        assert_eq!(cmd.extract_number(&Some("ADR-123".to_string()), &PathBuf::from("test.md")), Some(123));

        // Test extracting from filename
        assert_eq!(cmd.extract_number(&None, &PathBuf::from("0042-test.md")), Some(42));
        assert_eq!(cmd.extract_number(&None, &PathBuf::from("adr-007-test.md")), Some(7));

        // Test no number found
        assert_eq!(cmd.extract_number(&None, &PathBuf::from("no-number.md")), None);
    }
    ...
    fn test_status_badge_generation() {
        ...
        assert!(cmd.get_status_badge("accepted").contains("Status-Accepted-green"));
        assert!(cmd.get_status_badge("proposed").contains("Status-Proposed-yellow"));
        assert!(cmd.get_status_badge("rejected").contains("Status-Rejected-red"));
        assert!(cmd.get_status_badge("deprecated").contains("Status-Deprecated-orange"));
        assert!(cmd.get_status_badge("superseded").contains("Status-Superseded-lightgrey"));
        assert!(cmd.get_status_badge("custom").contains("Status-custom-blue"));
    }
    ...
    fn test_custom_template() {
        ...
        fs::write(&template_path, r#"# Custom ADR Index
