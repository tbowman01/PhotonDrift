...
#[cfg(feature = "plugins")]
use crate::plugins::{
    InstallationOptions, MarketplaceClient, PluginContext, PluginManager, SearchCriteria,
    SecurityPolicy, SortBy,
};
...
fn execute_list(detailed: bool) -> Result<()> {
    ...
    match PluginManager::new(&plugin_dir, context, security_policy) {
        ...
        Err(e) => {
            ...
            return Err(AdrscanError::IoError(format!(
...
fn execute_install(
    ...
    println!(
        "  Options: verify_signature={}, install_dependencies={}",
...
fn execute_toggle(plugin_id: &str, enable: bool, disable: bool) -> Result<()> {
    if enable && disable {
        return Err(AdrscanError::ConfigError(
            "Cannot both enable and disable a plugin".to_string(),
        ));
    }

    if !enable && !disable {
        return Err(AdrscanError::ConfigError(
            "Must specify either --enable or --disable".to_string(),
        ));
    }
...
fn execute_generate_ide(ide_type: &str, output: Option<&std::path::Path>) -> Result<()> {
    ...
    match ide_type.to_lowercase().as_str() {
        ...
        _ => {
            eprintln!(
                "Unsupported IDE type: {}. Supported: vscode, intellij, universal",
                ide_type
            );
            Err(AdrscanError::ConfigError(format!(
...
fn generate_vscode_integration(output_dir: &std::path::Path) -> Result<()> {
  ...
  "exclude": ["node_modules", ".vscode-test"]
    ...
    println!(
        "  ✅ VS Code extension files generated in: {}",
        output_dir.display()
    );
    println!("  📝 Next steps:");
...
fn generate_intellij_integration(output_dir: &std::path::Path) -> Result<()> {
    ...
    for (filename, content) in java_files {
        let file_path = output_dir
            .join("src/main/java/com/photondrift/intellij")
            .join(filename);
...
<idea-plugin>
    ...
    std::fs::write(
        output_dir.join("src/main/resources/META-INF/plugin.xml"),
        descriptor_xml,
    )?;

    println!(
        "  ✅ IntelliJ plugin files generated in: {}",
        output_dir.display()
    );
    println!("  📝 Next steps:");
    println!("     1. cd {}", output_dir.display());
...
fn generate_universal_integration(output_dir: &std::path::Path) -> Result<()> {
    ...
    std::fs::write(
        output_dir.join("lsp-config.json"),
...
(use-package lsp-mode
  ...
  (lsp-register-client
    ...
    println!(
        "  ✅ Universal LSP integration generated in: {}",
        output_dir.display()
    );
    println!("  📝 Next steps:");
    println!("     1. cd {}", output_dir.display());
