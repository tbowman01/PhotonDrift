...
impl ModelTrainer {
    ...
    pub fn train_model(
        ...
        log::info!("Starting training for {:?} model with {} samples", 
                  model_type, training_data.metadata.total_samples);
        
        // Check data quality
        self.validate_training_data(training_data)?;
        
        // Split data for validation
        let (train_data, val_data) = training_data.train_validation_split(self.config.validation_split);
        ...
        log::info!("Training completed in {:?}. Validation accuracy: {:.3}", 
                  start_time.elapsed(), metrics.validation_accuracy);
        ...
        for fold in 0..self.config.cv_folds {
            ...
            for i in 0..training_data.features.len() {
                if i >= start_idx && i < end_idx {
                    ...
                    train_data.add_sample(training_data.features[i].clone(), training_data.labels[i]);
                }
            }
    ...
    pub fn optimize_hyperparameters(
        ...
        log::info!("Starting hyperparameter optimization for {:?} with {} trials", 
                  model_type, n_trials);
    ...
    pub fn generate_report(&self) -> String {
        ...
        report.push_str(&format!("Total training sessions: {}\n", self.training_history.len()));
        
        if !self.training_history.is_empty() {
            let avg_accuracy: f64 = self.training_history.iter()
                .map(|s| s.metrics.validation_accuracy)
                .sum::<f64>() / self.training_history.len() as f64;
            
            report.push_str(&format!("Average validation accuracy: {:.3}\n", avg_accuracy));
            
            // Best performing model
            if let Some(best_session) = self.training_history.iter()
                .max_by(|a, b| a.metrics.validation_accuracy.partial_cmp(&b.metrics.validation_accuracy).unwrap()) {
    ...
    fn validate_training_data(&self, training_data: &TrainingData) -> DriftResult<()> {
        ...
        if training_data.metadata.total_samples < 10 {
            log::warn!("Training data has only {} samples, which may be insufficient", 
        ...
        if positive_ratio < 0.1 || positive_ratio > 0.9 {
            log::warn!("Training data is imbalanced: {:.1}% positive samples", 
...
mod tests {
    ...
    fn test_training_data_creation() {
        ...
        let training_data = TrainingData::from_drift_items(drift_items, &feature_extractor).unwrap();
