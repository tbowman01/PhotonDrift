...
impl OneClassSVMModel {
    ...
    fn normalize_features(&self, data: &[Vec<f64>]) -> Vec<Vec<f64>> {
        data.iter().map(|sample| {
            sample.iter().enumerate().map(|(i, &value)| {
    ...
    fn train_svm(&mut self, normalized_data: &[Vec<f64>]) -> DriftResult<()> {
        ...
        for iteration in 0..50 { // Limited iterations for performance
        ...
        self.decision_threshold = all_decision_values.get(outlier_index.min(n - 1))
            .copied()
            .unwrap_or(0.0);
    ...
    fn train_fallback(&mut self, normalized_data: &[Vec<f64>]) -> DriftResult<()> {
        ...
        let mut distances: Vec<f64> = normalized_data.iter()
        ...
        self.decision_threshold = distances.get(outlier_index.min(distances.len() - 1))
            .copied()
            .unwrap_or(0.5);
    ...
    fn fallback_anomaly_score(&self, features: &DriftFeatures) -> f64 {
        ...
        let pattern_weight = features.pattern_frequency * 0.15;
        let sentiment_weight = features.text_features.sentiment_score.abs() * 0.10;
        let tech_terms_weight = (features.text_features.tech_term_count as f64 / 20.0).min(1.0) * 0.15;
        
        (complexity_weight + file_count_weight + diversity_weight + 
         pattern_weight + sentiment_weight + tech_terms_weight).min(1.0)
    }
}
...
impl AnomalyModel for OneClassSVMModel {
    ...
    fn explain(&self, features: &DriftFeatures) -> Option<String> {
        if !self.is_trained {
            return Some("One-Class SVM analysis using fallback scoring (model not trained)".to_string());
        }
        ...
        if explanations.is_empty() {
            Some(format!(
                ...
                self.support_vectors.len(), self.nu, self.gamma
            ))
        } else {
            Some(format!(
                "SVM boundary analysis ({}): {}",
                if self.support_vectors.is_empty() { "fallback" } else { "trained" },
                explanations.join("; ")
            ))
        }
    }
...
impl LOFModel {
    ...
    pub fn train(&mut self, training_features: &[DriftFeatures]) -> DriftResult<()> {
        ...
        if self.n_neighbors >= feature_matrix.len() {
            ...
            log::warn!("Adjusted k to {} due to limited training data", self.n_neighbors);
        }
    ...
    fn calculate_k_distance(&self, point: &[f64], dataset: &[Vec<f64>], point_index: usize) -> DriftResult<f64> {
    ...
    fn calculate_lrd(&self, point: &[f64], dataset: &[Vec<f64>], point_index: usize) -> DriftResult<f64> {
    ...
    fn find_k_nearest_neighbors(&self, point: &[f64], dataset: &[Vec<f64>], point_index: usize) -> DriftResult<Vec<usize>> {
        // Find distances to all other points
        let mut distances: Vec<(f64, usize)> = Vec::new();
    ...
    fn calculate_lof_score(&self, features: &DriftFeatures) -> f64 {
        ...
        let neighbors_result = self.find_k_nearest_neighbors(&normalized_vec, &self.training_data, usize::MAX);
    ...
    fn explain(&self, features: &DriftFeatures) -> Option<String> {
        ...
        if explanations.is_empty() {
            Some(format!(
                ...
                lof_score, self.n_neighbors, self.training_data.len()
            ))
        } else {
            Some(format!(
...
mod tests {
    ...
    fn test_lof_k_distance_calculation() {
        ...
        let k_dist_small = lof.calculate_k_distance(&small_dataset[0], &small_dataset, 0).unwrap();
    ...
    fn test_lof_find_neighbors() {
        ...
        let neighbors = lof.find_k_nearest_neighbors(&dataset[0], &dataset, 0).unwrap();
