...
impl Snapshot {
    ...
    pub fn add_technology_match(&mut self, tech_match: &TechnologyMatch) {
        let id = format!(
            "tech_{}_{}",
            tech_match.file_path.replace('/', "_"),
            tech_match.line_number
        );
    ...
    fn update_statistics(&mut self) {
        ...
        self.statistics.files_scanned = self
            .entries
            .iter()
...
impl SnapshotComparison {
    ...
    pub fn technology_changes(
        ...
        let added_tech: Vec<&SnapshotEntry> = self
            .added_entries
            .iter()
            .filter(|e| matches!(e.entry_type, SnapshotEntryType::Technology))
            .collect();

        let removed_tech: Vec<&SnapshotEntry> = self
            .removed_entries
            .iter()
            .filter(|e| matches!(e.entry_type, SnapshotEntryType::Technology))
            .collect();

        let modified_tech: Vec<&SnapshotEntry> = self
            .modified_entries
            .iter()
