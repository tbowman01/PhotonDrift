...
use crate::drift::{
    DriftCategory, DriftItem, DriftLocation, DriftReport, DriftResult, DriftSeverity,
    ScanStatistics, Snapshot,
};
...
impl DriftDetector {
    ...
    async fn parse_single_adr(&self, file_path: &Path) -> DriftResult<AdrDecision> {
        ...
        let decision_type =
            self.determine_decision_type(&adr_doc.metadata.status, &adr_doc.content);
        ...
        match status_lower.as_str() {
            "accepted" => {
                if content_lower.contains("reject")
                    || content_lower.contains("deprecate")
                    || content_lower.contains("we will not")
                    || content_lower.contains("avoid")
                    || content_lower.contains("not use")
                {
                    DecisionType::Rejects
                } else if content_lower.contains("we will use")
                    || content_lower.contains("adopt")
    ...
    fn infer_categories_from_content(&self, content: &str) -> Vec<String> {
        ...
        if content_lower.contains("database")
            ...
            || content_lower.contains("persistence")
        {
        ...
        if content_lower.contains("cloud")
            || content_lower.contains("aws")
            || content_lower.contains("azure")
            || content_lower.contains("gcp")
        {
        ...
        if content_lower.contains("performance")
            ...
            || content_lower.contains("scaling")
            || content_lower.contains("latency")
        {
            categories.insert("performance".to_string());
        }

        if content_lower.contains("infrastructure")
    ...
    pub async fn detect_drift(
        ...
        if let Some(baseline) = baseline_snapshot {
            self.detect_snapshot_drift(current_snapshot, baseline, &mut report)
                .await?;
        }

        // 2. Detect drift from ADR decisions
        self.detect_adr_drift(current_snapshot, adr_decisions, &mut report)
            .await?;

        // 3. Detect new technologies not covered by ADRs
        self.detect_uncovered_technologies(current_snapshot, adr_decisions, &mut report)
            .await?;
        ...
        for removed_entry in &comparison.removed_entries {
            if let Some(ref technology) = removed_entry.technology {
                ...
                .with_suggested_action(
                    "Consider updating related ADRs to reflect this change".to_string(),
                );
            ...
            if let Some(ref detected_tech) = tech_entry.technology {
                ...
                if matching_decisions.is_empty() {
                    ...
                    for decision in matching_decisions {
                        if decision.decision_type == DecisionType::Rejects
    ...
    async fn detect_uncovered_technologies(
        ...
        for tech_entry in current_snapshot.technology_entries() {
            if let Some(ref technology) = tech_entry.technology {
                ...
                let is_covered = covered_technologies.iter().any(|covered_tech| {
                    tech_lower.contains(covered_tech) || covered_tech.contains(&tech_lower)
                });
        ...
        for (technology, count) in uncovered_count {
            if count >= 3 {
                // Only report if technology appears multiple times
                let drift_item = DriftItem::new(
...
"#,
        ...
        let mongo_decision = decisions
            .iter()
            .find(|d| d.title.contains("MongoDB"))
            .unwrap();
        assert_eq!(mongo_decision.decision_type, DecisionType::Rejects);
        assert!(mongo_decision
            .mentioned_technologies
            .contains(&"mongodb".to_string()));
    }
    ...
    fn test_infer_categories_from_content() {
        ...
        let database_content =
            "We will use PostgreSQL as our primary database for data persistence.";
...
"#,
        ...
        detector
            .detect_adr_drift(&snapshot, &decisions, &mut report)
            .await
            .unwrap();
        ...
        let conflict_item = critical_items
            .iter()
...
"#,
        ...
        let mut report = DriftReport::new(temp_dir.path().to_path_buf(), None);
        detector
            .detect_uncovered_technologies(&snapshot, &decisions, &mut report)
            .await
            .unwrap();
    ...
    async fn test_detect_snapshot_drift() {
        ...
        detector
            .detect_snapshot_drift(&current, &baseline, &mut report)
            .await
            .unwrap();
        ...
        assert!(added_items
            .iter()
            .any(|item| item.title.contains("New Library")));
        ...
        assert!(removed_items
            .iter()
            .any(|item| item.title.contains("Old Library")));
    }
...
"#,
        ...
        let report = detector
            .detect_drift(&current, None, &decisions, &patterns)
            .await
            .unwrap();
        ...
        assert!(critical_items
            .iter()
            .any(|item| item.category == DriftCategory::ConflictingTechnology
        ...
        assert!(report
            .severity_summary
            .contains_key(&DriftSeverity::Critical));
        assert!(report
            .category_summary
            .contains_key(&DriftCategory::ConflictingTechnology));
    }
}
