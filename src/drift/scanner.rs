...
impl CodebaseScanner {
    ...
    pub fn new() -> Self {
        Self {
            include_extensions: vec![
                ...
                "toml", "json", "yaml", "yml", "xml", "tf", "dockerfile",
                "md", "txt", "sql", "sh", "bat", "ps1", "rb", "php", "cs",
                "kt", "swift", "scala", "clj", "hs", "ml", "ex", "exs",
            ...
            .filter(|e| e.file_type().is_file())
        {
            ...
            if let Ok(metadata) = entry.metadata() {
                if metadata.len() > self.max_file_size {
                    log::debug!("Skipping large file: {} ({} bytes)", file_path.display(), metadata.len());
                    continue;
                }
            }
    ...
    fn classify_file(&self, file_path: &Path) -> SnapshotEntryType {
        ...
        match extension.as_str() {
            "rs" | "py" | "js" | "ts" | "java" | "go" | "c" | "cpp" | "h" | "hpp" |
            "kt" | "swift" | "scala" | "clj" | "hs" | "ml" | "ex" | "exs" | "rb" |
            "php" | "cs" => SnapshotEntryType::SourceFile,
            
            "toml" | "json" | "yaml" | "yml" | "xml" | "ini" | "conf" | "config" => SnapshotEntryType::Configuration,
    ...
    async fn get_git_info(&self, root_path: &Path) -> Result<(Option<String>, Option<String>), AdrscanError> {
