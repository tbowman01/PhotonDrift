...
mod dependency_validation_tests {
    ...
    fn test_wasmtime_api_compatibility() {
        ...
        assert!(
            engine_result.is_ok(),
            "Engine creation should work with new wasmtime version"
        );
    ...
    fn test_dirs_path_compatibility() {
        ...
        assert!(
            config_dir.is_some(),
            "Config directory should be resolvable"
        );
...
"#;
        ...
        assert!(
            sample_adr.contains("Accepted"),
            "Status parsing should work"
        );
