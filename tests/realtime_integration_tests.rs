...
use adrscan::realtime::events::{AlertLevel, PipelineEvent};
...
use adrscan::realtime::{
    EventBus, FileWatcher, IntelligentCache, MLPipeline, RealtimeConfig, RealtimeSystem,
    WebSocketServer,
};
...
impl RealtimeTestFixture {
    ...
    fn create_test_file(&self, index: usize, content: &str) {
        if index < self.test_files.len() {
            fs::write(&self.test_files[index], content).expect("Failed to write test file");
        }
    }

    fn modify_test_file(&self, index: usize, content: &str) {
        if index < self.test_files.len() {
            fs::write(&self.test_files[index], content).expect("Failed to modify test file");
        }
    }
...
async fn test_debounced_file_changes() {
    ...
    assert!(
        no_event.is_err(),
        "Should not receive additional events immediately"
    );
...
async fn test_intelligent_cache_functionality() {
    ...
    assert_eq!(
        cached_result.drift_probability,
        test_result.drift_probability
    );
...
async fn test_performance_requirements() {
    ...
    println!(
        "✅ File change detection latency: {}ms (requirement: <{}ms)",
        detection_time.as_millis(),
        fixture.config.max_latency_ms
    );
...
async fn test_high_throughput_file_changes() {
    ...
    println!(
        "✅ Processed {} file changes in {}ms ({:.1} files/sec)",
        events_received,
...
async fn test_memory_usage_bounds() {
    ...
    let stats = cache.get_stats().await;
    println!(
        "✅ Cache memory usage: {:.2}MB ({:.1}% of limit)",
