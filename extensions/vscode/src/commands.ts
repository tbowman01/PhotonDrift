...
export class AdrCommandProvider {
    constructor(
        private context: vscode.ExtensionContext,
        private adrExplorer: AdrExplorerProvider,
        private driftDetection: DriftDetectionProvider
    ) {}

    async initAdrStructure(): Promise<void> {
    ...
    async runInventory(): Promise<void> {
        ...
        try {
            ...
            const { stdout } = await execAsync(
                `"${adrscanPath}" inventory --format json`,
                { cwd: workspaceFolder.uri.fsPath }
            );
    ...
    async generateProposals(): Promise<void> {
        ...
        try {
            ...
            const adrscanPath = this.getAdrscanPath();
            const { stdout } = await execAsync(
                `"${adrscanPath}" propose --format json`,
                { cwd: workspaceFolder.uri.fsPath }
            );
    ...
    async createNewAdr(): Promise<void> {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            vscode.window.showErrorMessage('No workspace folder found');
            return;
        ...
        const adrNumber = await vscode.window.showInputBox({
            ...
            validateInput: (value) => {
                if (!value) return 'ADR number is required';
                if (!/^\d{4}$/.test(value)) return 'ADR number must be 4 digits (e.g., 0005)';
                return null;
            }
        });
    ...
    async toggleLspServer(): Promise<void> {
    ...
    async openDashboard(): Promise<void> {
        ...
        const panel = vscode.window.createWebviewPanel(
            'photondriftDashboard',
            'PhotonDrift Analytics Dashboard',
    ...
    async openAdr(adr: any): Promise<void> {
        if (adr.resourceUri) {
    ...
    async editAdr(adr: any): Promise<void> {
        await this.openAdr(adr);
    }

    async deleteAdr(adr: any): Promise<void> {
        if (!adr.resourceUri) return;

        const response = await vscode.window.showWarningMessage(
            `Are you sure you want to delete ${path.basename(adr.resourceUri.fsPath)}?`,
            'Delete',
            'Cancel'
        );
    ...
    private generateAdrContent(number: string, title: string, template: string): string {
...
`;
    ...
    private showInventoryResults(inventoryData: any): void {
        const panel = vscode.window.createWebviewPanel(
            'adrInventory',
            'ADR Inventory',
            vscode.ViewColumn.One,
    ...
    private showProposals(proposals: any[]): void {
...
<body>
    ...
    private getInventoryHtml(inventoryData: any): string {
        const adrs = inventoryData.adrs || [];
        const adrRows = adrs.map((adr: any) => `
            <tr>
                <td>${adr.number || ''}</td>
                <td>${adr.title || ''}</td>
...
<body>
    ...
    private getProposalsHtml(proposals: any[]): string {
